...
function parseChar(rawtext,endpoint=-1){
    ...
    while(!isend){
        ...
        // 如果这行有新效果 清除上一行临时效果
        if(linetxt[0]!=="load" && linetxt[0]!=="jump" && linetxt[0]!=="target"){
            tempfunc()
            tempfunc=()=>{}
        }
        // 人物加载
        if(linetxt[0]==="load" && ["spr","sprC","custom","customC","char","charC"].includes(linetxt[1])){
            res[linetxt[2]]=new Character(linetxt[2])
        }
        // 人物相关指令
        if(Object.keys(res).includes(linetxt[0])){
