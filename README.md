...
2. 使用浏览器访问`https://sanmusen214.github.io/arisstudio-blockly/`，即可开始构建
3. 当构建结束时，点击页面右上角`导出脚本`即可选择保存txt至ArisStudio的0Txt文件夹下。
4. 在ArisStudio中加载你刚才导出的txt脚本即可
...
3. 标有`(唯一)`字段的同一类块不能以同一id重复。举个例子，场上不能同时有两个`主线块1`。但是不同类的块不受此影响，场上可以同时有`定义支线201`，`运行支线201`

4. 字符串块在`其他块`大类的最后一位，如你所见，字符串块和变量块的插槽是一样的，这意味着他们可以等价使用。但是请注意变量块的名字不代表其值，其值是它被赋值的时候的右侧字符串块。推荐初始化中文名的变量名，并给其赋值为拼音。

5. 你可以将同一id的`运行支线块`视为`定义支线块`内的代码，引入`支线块`的目的是避免积木高度超标（尤其是有多项选择的情况下）

综上个人比较推荐的做法是使用`主线块1`赋值所有变量（如果你需要使用变量来代替字符串的话），然后使用`主线块2`放置`加载块`，然后在其内加载所有所需的资源。在之后的`主线块`中放置任何其他代码块
